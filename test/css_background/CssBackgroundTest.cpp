...
TEST(CssBackground, ParsesBoundedChapterHeadingSubset) {
  ...
  ASSERT_EQ(parser.loadFromStream(source, "OEBPS/Styles/chapter.css"), CssParser::ParseResult::Complete);
...
TEST(CssBackground, AcceptsBothTopKeywordOrdersAndHorizontalPositions) {
  ...
  ASSERT_EQ(parser.loadFromStream(source, "OPS/Css/book.css"), CssParser::ParseResult::Complete);
...
TEST(CssBackground, CacheRoundTripAndCorruptionRejection) {
  ...
  HalFile source(readFixture());
  ASSERT_EQ(writer.loadFromStream(source, "OEBPS/Styles/chapter.css"), CssParser::ParseResult::Complete);
  ...
  EXPECT_EQ(truncated.loadFromCache(), CssParser::CacheLoadResult::Invalid);

  Storage.clear();
  CssParser invalidWriter("/book");
  HalFile invalidSource(".x{background-image:url(../Images/x.png);background-repeat:no-repeat;"
                        "background-position:left top;}");
  ASSERT_EQ(invalidWriter.loadFromStream(invalidSource, "OPS/Css/book.css"), CssParser::ParseResult::Complete);
  ASSERT_TRUE(invalidWriter.saveToCache());
  auto& invalidCache = Storage.files.at("/book/css_rules.cache");

  // Cache layout: version, flags, rule count, path table, then rules.
  size_t cursor = sizeof(uint8_t) * 2 + sizeof(uint16_t);
  ...
  EXPECT_EQ(invalidId.loadFromCache(), CssParser::CacheLoadResult::Invalid);
}
