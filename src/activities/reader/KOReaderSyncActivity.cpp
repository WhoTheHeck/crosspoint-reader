...
uint32_t nextKosyncSessionId = 0;
...
KOReaderSyncActivity::KOReaderSyncActivity(GfxRenderer& renderer, MappedInputManager& mappedInput,
    ...
    : Activity("KOReaderSync", renderer, mappedInput),
      ...
      wifiSessionId(++nextKosyncSessionId) {
  if (wifiSessionId == 0) wifiSessionId = ++nextKosyncSessionId;
}
...
void KOReaderSyncActivity::onWifiSelectionComplete(const WifiCompletionReason reason) {
  if (reason != WifiCompletionReason::Connected) {
#if defined(CROSSPOINT_DIAGNOSTICS_X3)
    diagnosticJournal.recordKosyncWifiResult(
        wifiSessionId, static_cast<DiagnosticJournal::KosyncResult>(
                           diagnostics::kosyncResultCode(false, reason == WifiCompletionReason::UserCancelled)));
#endif
...
#if defined(CROSSPOINT_DIAGNOSTICS_X3)
  diagnosticJournal.recordKosyncWifiResult(
      wifiSessionId, static_cast<DiagnosticJournal::KosyncResult>(diagnostics::kosyncResultCode(true, false)));
#endif
  performSync();
}
...
void KOReaderSyncActivity::onEnter() {
...
#if defined(CROSSPOINT_DIAGNOSTICS_X3)
    diagnosticJournal.recordKosyncWifiResult(
        wifiSessionId, static_cast<DiagnosticJournal::KosyncResult>(diagnostics::kosyncResultCode(false, false)));
#endif
    requestUpdate();
    return;
  }
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    onWifiSelectionComplete(WifiCompletionReason::Connected);
    return;
  }
  ...
  startActivityForResult(std::make_unique<WifiSelectionActivity>(renderer, mappedInput, true,
                                                                 WifiSelectionOrigin::KOReaderSync, wifiSessionId),
                         [this](const ActivityResult& result) {
                           if (!std::holds_alternative<WifiResult>(result.data)) {
                             onWifiSelectionComplete(WifiCompletionReason::Failed);
                             return;
                           }
                           onWifiSelectionComplete(std::get<WifiResult>(result.data).reason);
                         });
}
